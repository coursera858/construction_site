...
import TransactionForm from "../../forms/Assets/TransactionForm"
import toast from "react-hot-toast"
import {
  MdAccountBalanceWallet, MdCallReceived, MdCallMade,
  MdTrendingUp, MdTrendingDown, MdOutlineAccountBalance
} from "react-icons/md"
...
const reducer = (state, action) => {
  switch (action.type) {
    case "SET_DATA":
      return { ...state, booking: action.payload.booking, transactions: action.payload.transactions }
    case "SET_LOADING":
...
const ViewBookingdetails = () => {
  ...
  const { booking, transactions, isLoading, addModal, editModal, selectedTransaction } = state
  const [typeFilter, setTypeFilter] = useState('All')

  const fetchDetails = useCallback(async () => {
    dispatch({ type: "SET_LOADING", payload: true })
    const response = await getRequest(`/booking/${id}/transaction/`)
    if (response.success) {
      dispatch({ type: "SET_DATA", payload: { booking: response.data.booking, transactions: response.data.transactions } })
    }
  ...
  const handleAddTransaction = useCallback(async (data) => {
    const response = await postRequest(`/booking/${id}/transaction/`, data)
  ...
  const handleEditTransaction = useCallback(async (data) => {
    const response = await patchRequest(`/booking/${id}/transaction/${selectedTransaction._id}`, data)
  ...
  const handleDeleteTransaction = useCallback(async (transactionId) => {
    const response = await deleteRequest(`/booking/${id}/transaction/${transactionId}`)
    if (response.success) { fetchDetails(); toast.success("Transaction deleted") }
    else toast.error("Failed to delete transaction")
  }, [id, fetchDetails])

  if (isLoading && !booking) return <span>Loading...</span>

  const totalIncome  = transactions.filter(t => t.type === 'income').reduce((acc, t) => acc + t.amount, 0)
  const totalExpense = transactions.filter(t => t.type === 'expense').reduce((acc, t) => acc + t.amount, 0)
  const balanceDue   = (booking?.total_Amount || 0) - totalIncome
  ...
  return (
    <div id="wrapper">
      ...
      {/* ── 4 Stat Tiles ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', padding: '0 0 24px' }}>
        <StatTile icon={<MdAccountBalanceWallet />}    label="Total Amount"       value={`₹${(booking?.total_Amount || 0).toLocaleString()}`} variant="neutral" />
        <StatTile icon={<MdCallReceived />}            label="Payments Received"  value={`₹${totalIncome.toLocaleString()}`}                   variant="income" />
        <StatTile icon={<MdCallMade />}                label="Total Expenses"     value={`₹${totalExpense.toLocaleString()}`}                   variant="expense" />
        <StatTile icon={<MdOutlineAccountBalance />}   label="Balance Due"        value={`₹${balanceDue.toLocaleString()}`}                    variant={balanceDue <= 0 ? 'income' : 'warning'} />
        <StatTile icon={profit >= 0 ? <MdTrendingUp /> : <MdTrendingDown />} label={profit >= 0 ? 'Profit' : 'Loss'} value={`₹${Math.abs(profit).toLocaleString()}`} variant={profit >= 0 ? 'income' : 'expense'} />
      </div>

      {/* ── Booking Info + Category Chart ── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '24px' }}>
        ...
        <div className="details-section">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            ...
            <DetailRow label="Phone" value={booking?.phone_number} />
            <DetailRow label="Asset" value={`${booking?.asset?.name} (${booking?.asset?.asset_type})`} />
            <DetailRow label="Linked Project" value={booking?.project?.name || 'None'} />
            <DetailRow label="Rented Date" value={new Date(booking?.rented_date).toLocaleDateString()} />
            <DetailRow label="Returned Date" value={booking?.returned_date ? new Date(booking?.returned_date).toLocaleDateString() : 'N/A'} />
            <DetailRow label="Return Status" value={<span className={`status ${booking?.returned_status}`}>{booking?.returned_status}</span>} />
            <DetailRow label="Payment Status" value={<span className={`status ${booking?.payment_status}`}>{booking?.payment_status}</span>} />
          </div>
        </div>
      ...
      <Modal isOpen={editModal} setIsOpen={() => dispatch({ type: 'CLOSE_EDIT_MODAL' })}>
        {selectedTransaction && (
          <TransactionForm data={selectedTransaction} handleTransaction={handleEditTransaction} />
        )}
      </Modal>
    </div>
  )
